...
namespace async {
    ...
    inline void connect_native_options::set_server_url(const memepp::string& _server_url)
    {
        ...
        server_url_ = connect_options::convert_url(_server_url, !!ssl_opt_);
        // The server URI is passed to MQTTAsync_createWithOptions() at init() time.
        // MQTTAsync_connectOptions does not have a single-server URI field (only
        // the multi-server serverURIs[] array + serverURIcount).  For single-server
        // configurations the create-time URI is authoritative.
    }
